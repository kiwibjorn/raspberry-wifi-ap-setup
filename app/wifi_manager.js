...
module.exports = function() {
    ...
    _update_tasmotas = function(ip_address,callback) {
      exec("nmap -sn " + ip_address + "/24", function(error, stdout, stderr) {
          var m = matchAll(stdout,/report for ([\d\.]+)\n.*\n.*Espressif/ig).toArray();
	  for(var tasmota_ip of m){
            callback("Found tasmota to configure: " + tasmota_ip);
	    exec("curl  http://" + tasmota_ip + "/cm?cmnd=MqttHost%20" + ip_address, function(error, stdout, stderr) {
		return null;
	    });
	    exec("curl  http://" + tasmota_ip + "/cm?cmnd=MqttUser%20mqtt-user", function(error, stdout, stderr) {
                return null;
            });
            exec("curl  http://" + tasmota_ip + "/cm?cmnd=MqttPassword%20R0arpower", function(error, stdout, stderr) {
                return null;
            });
	  }
    ...
    return {
        ...
        update_tasmotas:        _update_tasmotas,
