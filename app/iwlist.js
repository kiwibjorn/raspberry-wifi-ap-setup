        ...
        function append_previous_cell() {
            if (current_cell != null && interface_entry != null) {
                if (typeof(current_cell["ssid"]) != "undefined" &&
                    current_cell["ssid"] != "" ) {
	            var scan_results = interface_entry["scan_results"];
	            for(var i = 0; i < scan_results.length; i++){
			if(scan_results[i]["ssid"] == current_cell["ssid"]){
			  if(scan_results[i]["signal_strength"] > current_cell["signal_strength"]){
			    interface_entry["scan_results"] = scan_results.splice(i, 1);
